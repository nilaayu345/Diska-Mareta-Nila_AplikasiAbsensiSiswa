...
class TeacherSubjectsAndBatches {
  ...
  final CollectionReference _teachers =
      Firestore.instance.collection('/teachers-data');
  ...
  Future<String> addSubject(String subject) async {
    try {
      ...
      await _teachers
          .document(user.email)
          .setData({subject: true}, merge: true);
      return 'Success';
    } catch (e) {
      print(e.toString());
      return null;
  ...
  Future deleteSubject(String subject) async {
    try {
      ...
      await _teachers
          .document(user.email)
          .setData({subject: FieldValue.delete()}, merge: true);
      await _teachers
          .document(user.email)
          .collection(subject)
          .getDocuments()
  ...
  Future<String> addStudent(
    ...
    try {
      await _teachers
          ...
          .collection(subject)
          .document(batch)
          .setData({studentEmail: true}, merge: true);
      CollectionReference _students =
          Firestore.instance.collection('students-data');
  ...
  Future<String> deleteStudent(
    ...
    try {
      await _teachers
          ...
          .collection(subject)
          .document(batch)
          .setData({studentEmail: FieldValue.delete()}, merge: true);
      await _teachers
          .document(user.email)
          .collection(subject)
          .document(batch)
      ...
      if (studentDetails.isNotEmpty) {
        ...
        for (String key in keys) {
          if (studentDetails[key]['teacherEmail'] == user.email) {
            await _students
                .document(studentEmail)
                .setData({key: FieldValue.delete()}, merge: true);
          }
        }
      }
      return 'Success';
    ...
    try {
      ...
      for (String studentEmail in attendance.keys) {
        await attendanceReference
            .document(studentEmail)
            .setData({dateTime: attendance[studentEmail]}, merge: true);
      }
      return 'Success';
    } catch (e) {
      print(e.toString());
  ...
  Future<List<String>> getBatches(String subject) async {
    try {
      List<String> batches = [];
      QuerySnapshot qs = await _teachers
          .document(user.email)
          .collection(subject)
          .getDocuments();
